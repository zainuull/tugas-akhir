...
function stableSort<T extends { operator?: string }>(
  array: readonly T[],
  ...
  stabilizedThis.sort((a, b) => {
    ...
    // Handle undefined values for operator
    if (a[0].operator === undefined && b[0].operator === undefined) {
      return 0;
    } else if (a[0].operator === undefined) {
      return 1;
    } else if (b[0].operator === undefined) {
      return -1;
    }
    return 0;
  });
...
export default function ListCredit() {
  ...
  const [orderBy, setOrderBy] = useState<keyof IData>('time');
  const [selected, setSelected] = useState<readonly number[]>([]);
  const [page, setPage] = useState(0);
  const router = useRouter();
  const rows = mockCredit;
  ...
  const handleDetail = (id: number) => {
    router.push(`history/detail/credit/${id}`);
  };
              ...
              {visibleRows.map((row, index) => {
                ...
                return (
                  <TableRow
                    ...
                    <TableCell align="center">{row.total_denomination}</TableCell>
                    <TableCell align="center">
                      {row.total_price ? row.total_price.toLocaleString('id-ID') : '-'}
                    </TableCell>
