...
interface IListFailed {
  operator: IDataOperatorModel;
  setOperator: Function;
}

const ListFailed = (props: IListFailed) => {
  const { operator, setOperator } = props;
  return (
    <div className="w-full min-h-40 bg-white flex flex-col rounded-lg gap-y-2">
      <h1 className="px-4 text-lg font-semibold text-primary mt-4">
        LIST OF ALL CARDS THAT FAILED INJECTION
      </h1>
      {/* Dropdown */}
      ...
      <span className="w-full text-end mb-4">
        <button className="button w-1/2">Re-Inject</button>
      </span>
    </div>
  );
};
