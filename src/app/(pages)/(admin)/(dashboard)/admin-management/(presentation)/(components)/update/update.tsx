...
interface IUpdateAdmin {
  ...
  dataInput: IDataAdmin;
  setDataInput: Function;
  fetchData: Function;
}

const UpdateAdmin = (props: IUpdateAdmin) => {
  const { updateDataAdmin } = VM();
  ...
  const handleSubmit = () => {
    ...
    notifyService.confirmationUpdate().then((res) => {
      if (res) {
        updateDataAdmin(dataInput?.id || '', payload)
          .then(() => {
            fetchData();
            handleClose();
      ...
      <div className="w-full flex flex-col px-4 pt-2">
        ...
        <form className="mt-2 flex flex-col gap-y-2">
          ...
          <div className="flex flex-col gap-y-1 text-sm relative">
            <label htmlFor="old_password" className="font-medium">
              Password Lama
            </label>
            <input
              id="old_password"
